def possible_games(input_string, red_cubes, green_cubes, blue_cubes):
    games = parse_input(input_string)
    count = 0
    for game in games:
        if is_possible(game, red_cubes, green_cubes, blue_cubes):
            count += 1
    return count
    ...
    for line in input_string.strip().split("\n"):
        if line.startswith("Game"):
            ...
            for cube in cubes:
                ...
                if cube:
                    colors = cube.split(",")
                    cube_set.append((colors[1].strip(), int(colors[0])))
            current_game["cubes"].append(cube_set)
...
result = possible_games(input_string, red_cubes, green_cubes, blue_cubes)
print(result)
